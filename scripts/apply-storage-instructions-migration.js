...
async function applyStorageInstructionsMigration() {
  try {
    console.log('Starting storage_instructions column migration...');
    
    // Read the migration file
    const migrationPath = path.join(process.cwd(), 'migrations', 'add_storage_instructions_column.sql');
    ...
    try {
      ...
      try {
        ...
        const { data, error: checkError } = await supabase
          ...
          .eq('column_name', 'storage_instructions');
...
// Run the migration
applyStorageInstructionsMigration(); 